...
bot.onText(/\/add (.+)/, async (msg, match) => {
  ...
  bot.editMessageText(
    saved
      ? `✅ *تم إضافة المفتاح بنجاح!*\n\n💾 تم الحفظ على GitHub تلقائياً\n🔑 المجموع: ${state.getTokens().length} مفاتيح`
      : `✅ *تم إضافة المفتاح في الذاكرة*\n\n⚠️ لم يتم الحفظ على GitHub\nتأكد من إضافة GITHUB\\_TOKEN في Render\n🔑 المجموع: ${state.getTokens().length} مفاتيح`,
    { chat_id: msg.chat.id, message_id: savingMsg.message_id, parse_mode: 'Markdown' }
  );
});
...
bot.onText(/\/remove (\d+)/, async (msg, match) => {
  ...
  if (index < 0 || index >= tokens.length) {
    return bot.sendMessage(msg.chat.id, `❌ رقم غير صحيح.\nالمفاتيح من 1 إلى ${tokens.length}`);
  }
...
bot.onText(/\/status/, async (msg) => {
  ...
  try {
    ...
    bot.sendMessage(msg.chat.id,
      `📊 *حالة الخدمة:*\n\n` +
      `${data.status === 'ok' ? '✅ الخدمة تعمل بشكل طبيعي' : '⚠️ مشكلة في الخدمة'}\n\n` +
    ...
    bot.sendMessage(msg.chat.id, '❌ الخدمة لا تستجيب حالياً\n\nقد تكون في وضع السكون (Render Free Tier)');
  }
});
